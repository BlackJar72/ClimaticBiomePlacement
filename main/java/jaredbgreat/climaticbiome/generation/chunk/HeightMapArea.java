...
public class HeightMapArea extends AbstractWeaklyCacheable {
	...
	public HeightMapArea(int x, int z, SpatialHash rand, HeightMapManager manager) {
    	...
    	HeightNoiseMap noise1 = new HeightNoiseMap(BSIZE, BSIZE, BSIZE, 8, SCALE);
    	HeightNoiseMap noise2 = new HeightNoiseMap(BSIZE, BSIZE, BSIZE, 8, SCALE);
    	heightMap = noise1.process(rand, x * BSIZE, z * BSIZE);
    	other = noise2.process(new SpatialHash(rand.longFor(8, 6, 7, 5)), x * BSIZE, z * BSIZE);
	}
	...
	public int[][] getChunkHieghts(int x, int z, float[] biomeData, float[] vol) {
    	int[][] out = new int[4][256];
    	int startx = ModMath.modRight(x, CSIZE) * 16;
    	int startz = ModMath.modRight(z, CSIZE) * 16;
    	int endx   = startx + 16;
